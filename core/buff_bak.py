...
class Modifier(object):
    def __init__(this, host):
        this.host = host
        this.type_mods = {}
        this.cache = {}  # type: cache_value(-1:dirty -2:cannot cache)
...
class _Modifier(object):
    ...
    def set(this, value):
        ...
        if cache[this.mod_type] != -2:
            cache[this.mod_type] = -1
        this.mod_value = value
    ...
    def on(this, value=None):
        ...
        cache = this._static.cache
        if this.mod_type in cache:
            if cache[this.mod_type] == -2: # mod are always dirty
                return this  

        if this.get!=this.__get:
            cache[this.mod_type] = -2
        else:
            cache[this.mod_type] = -1
        return this
